...
const bindEventFn = (event, boundEvent, leaf, data) => {
  let resolvedValue = bindDataFn(boundEvent, leaf, data);

  if (resolvedValue === undefined && leaf?.model?.originalModelName) {
    const propName = boundEvent.match(/\{\*:([^}]+)\}/)?.[1];

    // Check if this property was meta-transformed
    const originalPropName = leaf.meta
      ? Object.entries(leaf.meta).find(([k, v]) => v === propName)?.[0] || propName
      : propName;

    // Try both transformed and original property names
    const propsToCheck = [propName, originalPropName].filter((p, i, arr) => arr.indexOf(p) === i);

    for (const prop of propsToCheck) {
      const dataKey = `${leaf.model.originalModelName}--${prop}`;
      const originalModelData = getData([leaf.model.originalModelName]);

      if (originalModelData[dataKey] !== undefined) {
        resolvedValue = originalModelData[dataKey];
        console.log(`[i] Using fallback '${prop}' = '${resolvedValue}' from original model`);
        break;
      }
    }
  }

  return boundEvent.replace(boundEvent, resolvedValue || "undefined");
};
...
const bindListener = (listener, leaf, attributes, data, event = undefined, boundEvents = [], modelBinding = null) => (
  ...
  boundEvents && boundEvents.length
    ? boundEvents
        ...
        .forEach((unboundEvent, index) => (event = event.replace(boundEvents[index], unboundEvent)))
    : null,
