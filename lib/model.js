...
const getModelBinding = (field, multiple = undefined) =>
  typeof field !== "string"
    ...
    : ((multiple = field
        .split(",")
        .filter((f) => (f == "undefined" ? console.log(new Error()) : null, f !== "undefined"))),
  ...
  rootMethod.models[baseName] = function () {
    ...
    this.getMetaTransform = function (id, metaKey) {
      ...
      const modelMeta = keys
        .filter((m) => m.indexOf(metaKey) > -1)
        .reduce((acc, target) => ({ ...acc, [target]: name + "--" + Meta[target] }), {});
    ...
    this.append = async function (data) {
      const model = this;
      const dataName = this.getModelName();
      const modelName = dataName.replace("--", "");
    ...
    this.shouldUnload = () => {
      ...
      return len && keys.filter((key) => key.match(/([A-Z])\w+--/gi, "")[0] === modelName).length;
    };
