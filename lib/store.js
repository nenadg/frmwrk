...
const pointInTime = (key, at = 0, keys, pointer = 0) => (
  (pointer = keys.length - 1), /* pointer + at < 0 ? 0 : */ pointer + at
);
...
const compact = (acc, target) => Object.assign(acc, target);
...
const getData = (modelNames, at = 0, mutable) => (
  (mutable = Object.keys(Data)
    .filter((key) => ((key = key.substr(0, key.indexOf("--"))), modelNames.indexOf(key) > -1))
    .map(mapData.bind(null, at))
    .reduce(compact, {})),
  mutable
);

const reduceIndexes = (data, order, acc, target, i, arr, reduced) => (
  (reduced = order.slice(target, arr[i - 1]).reduce(reduceSlice, data)),
  arr[i - 1] !== undefined ? (!acc ? reduced : reduced && reduced[acc]) : !acc ? reduced : acc[reduced]
);

const reduceSlice = (a, t) => a && a[t];
...
const matchExpression = (candidate, leaf) => {
  let boundIndex = leaf.boundIndex;

  if (!boundIndex) {
    let bindingLeaf = getBindingLeaf(leaf);
    boundIndex = bindingLeaf && bindingLeaf.boundIndex;
  }

  const rebound = rebindValue(candidate, leaf);
  const dataMatch = rebound.match(mappedRe)[0];
  // const quadraticExp = dataMatch.match(quadraticRe) !== null;
  // const matrixExp = dataMatch.match(matrixMatchRe) !== null;
  const arrayExp = dataMatch.match(arrayRe) !== null || boundIndex !== undefined;

  let match = "";

  if (arrayExp) {
    let horizontalIndex = boundIndex && Number(boundIndex.split(".")[1]);
    let indexMatch;
    let indexLeaf;
    let indexRoot;
    let verticalIndex;

    if (dataMatch.match(quadraticRe) !== null) {
      // quadraticMatch
      indexMatch = dataMatch.replace(quadraticRe, "");
      indexLeaf = getLeafByBoundIndex(leaf, indexMatch);
      indexRoot = indexLeaf && indexLeaf.boundIndex.split(".");
      verticalIndex = indexRoot && Number(indexRoot[1]);
      return getMatch(dataMatch, horizontalIndex, verticalIndex);
    }

    indexMatch = dataMatch.replace(matrixMatchRe, "");
    indexLeaf = getLeafByBoundIndex(leaf, indexMatch);
    indexRoot = indexLeaf && indexLeaf.boundIndex.split(".");
    verticalIndex = indexRoot && Number(indexRoot[1]);
    match = getMatch(dataMatch, horizontalIndex, verticalIndex);

    if (dataMatch.match(matrixMatchRe) !== null) {
      // matrixMatch
      if (verticalIndex !== undefined) {
        let bindingLeaf = getBindingLeaf(getLeaf(indexLeaf.parent));
        boundIndex = bindingLeaf && bindingLeaf.boundIndex;
        horizontalIndex = boundIndex && Number(boundIndex.split(".")[1]);
...
const bindToData = (leaf, attributes) => {
  ...
  for (; j < len; j++) {
    ...
    let metaMatches = model.getMetaTransform(leaf.elementId, matched);
    ...
    if (Object.keys(metaMatches).length) {
      ...
      if (metaRoot) {
        ...
        let metaMatch = Object.keys(metaMatches).filter((mm) => mm === matched + "|" + metaRootId)[0];
...
// const getBindableFields = (attributes, keys = [], values = []) => (
//   keys = Object.keys(attributes),
//   values = Object.values(attributes),
//   values.map((value, index) => isDataField(value) ? keys[index] : null)
//     .filter(f => f !== null)
//   );

// const getBindableFields = attributes =>
//   Object.keys(attributes)
//     .map(key => (isDataField(attributes[key]) ?
//       attributes[key].match(dataRe).map(k => key) : null))
//     .flat()
//     .filter(key => key !== null);
...
const rebindValue = (value, leaf) => {
  ...
  const bindingModel = leaf.model;
  const bindingLeaf = leaf.bindingLeaf; // getBindingLeaf(leaf);
  let boundRoot = undefined;
  let key = undefined;

  if (bindingLeaf) {
    boundRoot = bindingLeaf.boundRoot;
    key = boundRoot.substr(0, boundRoot.indexOf("--"));
  }

  if (bindingModel) {
    key = bindingModel.getName();
  }
