...
// Memoization helper that maintains functional purity
...
// Optimized version that maintains functional style
const getDataKeys = memoizeWithTTL((key) => (Data[key] ? Data[key].__marrow_keys : [0]));

// Enhanced pointInTime with better bounds checking while keeping pure function
const pointInTime = (key, at = 0, keys) =>
  ((pointer) => {
    const targetPoint = pointer + at;
    return targetPoint < 0 ? 0 : targetPoint;
  })(keys.length - 1);

const getTimestamps = (key, at = 0) => Object.values(getDataKeys(key)).reverse()[at];

// Optimized but still pure version check
const getVersionAt = (key, at) => (Data[key] ? Data[key][at] : undefined);

// Enhanced getProperVersion maintaining functional paradigm
const getProperVersion = (at, key, keys) => ({ [key]: getVersionAt(key, keys[pointInTime(key, at, keys)]) });

// This handles getting individual property versions
const getCurrentVersions = (modelName) => {
  const modelKeys = Object.keys(Data).filter((key) => key.startsWith(modelName + "--"));
  return modelKeys.reduce((acc, key) => {
    acc[key] = Data[key].__marrow_generation;
    return acc;
  }, {});
};

// Get the highest generation among all properties
const getHighestGeneration = (modelName) => {
  const versions = getCurrentVersions(modelName);
  return Math.max(...Object.values(versions));
};

const canUndo = (modelName) => {
  const versions = getCurrentVersions(modelName);
  // Check if any property has more than one version
  return Object.values(versions).some((gen) => gen > 0);
};

const canRedo = (modelName) => {
  const currentVersions = getCurrentVersions(modelName);
  // Check if any property can move forward
  return Object.keys(currentVersions).some((key) => {
    const maxVersion = Data[key].__marrow_keys.length - 1;
    return currentVersions[key] < maxVersion;
  });
};

// For undo/redo operations, we need to track which properties can move
const getVersionableProperties = (modelName, direction = -1) => {
  const currentVersions = getCurrentVersions(modelName);

  return Object.keys(currentVersions).filter((key) => {
    const currentGen = currentVersions[key];
    if (direction < 0) {
      // For undo: can property go back?
      return currentGen > 0;
    } else {
      // For redo: can property go forward?
      const maxVersion = Data[key].__marrow_keys.length - 1;
      return currentGen < maxVersion;
    }
  });
};

// Optimized mapData that maintains functional paradigm
const mapData = (at, key, keys = []) =>
  ((keys = getDataKeys(key)) => getProperVersion(typeof at === "boolean" ? 1 - keys.length : at, key, keys))();

// Enhanced getData with better performance while keeping functional style
const getData = (modelNames, at = 0) =>
  Object.keys(Data)
