...
import { removeRelated, removeLeaf } from "./tree.js";
...
const updateElement = (current, previous, leaf) => {
  ...
  if (!element) {
    return handleMissingElement(current, previous, bindables, leaf);
  }
 ...
 */
const handleMissingElement = (current, previous, bindables, leaf) => {
  const dataAlive = bindables.indexOf("data-alive");
...
const handleAttributeUpdate = (element, attribute, value, previousValue, props, prop, current, previous, leaf) => {
  let attr = attribute.indexOf('data-') > -1 ? 'data' : attribute;
