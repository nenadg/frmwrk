...
const metrics = {
  lastFrameTime: 0, // Time taken to render the last frame
  avgFrameTime: 16, // Rolling average of frame times (starts assuming 60fps)
  chunkHistory: [] // Keep track of recent chunk performance
};
...
const mapChild = (childType, parentLeaf) =>
  typeof childType !== "string"
    ...
    : {
        ...
        ...(childType.match(dataRe) ? { ...mapFloatingChild(childType, parentLeaf), floating: childType } : {}),
...
const applyChunk = async (chunk, index, arr, startTime = performance.now(), endTime = null, frameTime = null) => (
  await Promise.all(chunk.map(appendRenderPeace).map(applyMounting)),
  (endTime = performance.now()),
  (frameTime = endTime - startTime),
  (metrics.lastFrameTime = frameTime),
  frameTime < 100 ? (metrics.avgFrameTime = metrics.avgFrameTime * 0.9 + frameTime * 0.1) : null,
  metrics.chunkHistory.push({ size: chunk.length, time: frameTime }),
  metrics.chunkHistory.length > 10 ? metrics.chunkHistory.shift() : null
);

// This function calculates the optimal chunk size based on current system performance
const calculateOptimalChunkSize = (queueLength) => {
  // We're aiming to stay under 4ms per frame to leave room for other browser work
  const targetFrameTime = 4;
  const currentPerformance = metrics.avgFrameTime;

  // Start with the current approach as a baseline
  let baseSize = Math.floor((queueLength * 2) / 100) || 1;

  if (currentPerformance > targetFrameTime) {
    // We're taking too long, reduce chunk size
    const reductionFactor = targetFrameTime / currentPerformance;
    baseSize = Math.max(1, Math.floor(baseSize * reductionFactor));
  } else if (currentPerformance < 1) {
    // We're very fast, we can process more
    // But let's be conservative and not increase too dramatically
    const increaseFactor = Math.min(3, 1 / currentPerformance);
    baseSize = Math.min(queueLength, Math.floor(baseSize * increaseFactor));
  }

  // Add safety bounds
  baseSize = Math.max(1, Math.min(baseSize, Math.floor(queueLength / 2)));
...
const chunkizeForRender = () => {
  ...
  const optimalChunkSize = calculateOptimalChunkSize(len);

  queueSlice
    .reduce((acc, item, index) => {
      const chunkIndex = Math.floor(index / optimalChunkSize);
...
const render = (configuration) => {
  ...
  if (!element) {
    ...
    if (config[componentType].oninitelement) {
      let modelBinding = getModelBinding(config[componentType].oninitelement);

      if (modelBinding) {
        if (!Array.isArray(modelBinding)) {
          modelBinding = [modelBinding];
        }

        for (let mb of modelBinding) {
          mb.fn(mb.model, leaf.current, element);
        }
      }
    }
