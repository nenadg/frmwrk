...
const renderOut = (renderPeace) => {
  ...
  if (parent && element) {
    ...
    // element.dataset['mid'] = renderPeace.leaf.elementId;
    parent.insertBefore(element, beforeChild);

    if (leaf.dataMatch && leaf.dataMatch.length) {
      // if (!leaf.current) {
      //   const model = leaf.model;
      //   const data = model ? getData([leaf.model.getName()]) : undefined;
      //   const previousData = model ? getData([leaf.model.getName()], -1) : undefined;
      //   // NOTE: this is for binding directly from model {Model--peaceOfData}
      //   // but it won't provide update mechanism
      //   // let directBind = leaf.dataMatch.filter(bind => !bind.startsWith(leaf.model.getName()));

      //   // if (directBind.length) {
      //   //   let models = directBind.map(bind => getModel(bind.split('--')[0]));
      //   //   let auxData = models.map(model => getData([model.getInstance().getName()]));
      //   //   let auxPrevData = models.map(model => getData([model.getInstance().getName()], -1));

      //   //   auxData.forEach(aux => Object.assign(data, aux));
      //   //   auxPrevData.forEach(aux => Object.assign(previousData, aux));
      //   // }
...
const renderBound = (componentType, config) => {
  ...
  // let timeoutFn = function(child) {
  //   render(child)
  // };

  // children.forEafch(child => setTimeout(timeoutFn.bind(null, child), 0));
  ...
  chunks.map(
    (
      chunk // chunk.map(render));
    ) => setTimeout(timeoutFn.bind(null, chunk), 0)
  );
};

const getDataKey = (key, parentLeaf) => key.replace("*:", getParentsModel(parentLeaf).getName() + "--");

const solveData = (componentType, parentLeaf, key = undefined) => (
  (key = getBoundRoot(componentType)),
  componentType.indexOf("*:") > -1 ? (key = getDataKey(key, parentLeaf)) : null,
  key ? (getData([key.substr(0, key.indexOf("--"))])[key] || []).map((d, i) => ({ datum: d, index: i })) : undefined
);
...
const prepareBoundChildren = (componentType, config, parentLeaf, templateId) =>
  (solveData(componentType, parentLeaf) || []).map((data) =>
    prepareBoundChild(componentType, config, parentLeaf, getBoundRoot(componentType), data.index, templateId)
  );
...
const renderHTML = (config, leaf, data, componentType) => {
  ...
  const rawAttributes = Object.assign({}, config[componentType]);
  // getElementProperties(leaf, componentType);
  ...
  // typeof boundAttributes[key] === 'string' &&
  // boundAttributes[key].match(dataRe) ?
  //   (attributes[key] = boundAttributes[key].replace(dataRe, '')) :
  //     null));
